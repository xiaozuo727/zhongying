...
//			传用户等级
		created: function(){
			...
			this.lv1=jin*100*5/1000+'元';
			this.lv2=jin*100*6/1000+'元';
			this.lv3=jin*100*7/1000+'元';
			var data={
				params:{}
			};
		...
		methods:{
			...
			isApply(e){
				...
				var config={headers: {'X-Requested-With': 'XMLHttpRequest'},emulateJSON: true};
				this.axios.get(this.Global.hostUrl+'mobile/Creditcard/creditcard',data,config).then(res=>{
					// console.log(res.data);
					if(res.data.error=='success'){
						window.location.href=res.data.data.result.applyUrl;
						th.loading=false;
					}else{
						alert(res.data.msg);
                        th.loading=false;
