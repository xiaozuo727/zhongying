<template>
	...
	<div class="content">
		<div class="border-b-gray-5 mar-t-1 pad-b-1">
			<div class="bg-green-gradient2 border-radius-5 col-9 auto color-white">
				<div class="pad-6 line-height-2">
					<p class="f14 hidden">
						<img src="../assets/img/huang1.png" alt="" width="30" />普通会员
						<span class="f18 fr"><b>￥ 198.0</b></span>
					</p>
					<p>
						时限：永久有效
						<span class="fr">推荐指数：★★★</span>
					</p>
					<p class="">权限：升级普通会员LV1，无卡支付收益，贷款收益、招商收益、办卡收益。</p>
				</div>
        ...
        if(this.paymethd=='wxpay'){
          ...
          this.axios.get(this.Global.hostUrl+'mobile/Wechatpay/createorder',data,config).then((res) => {
            ...
            if(res.data.error=="success"){
              window.android.wxpay(res.data.data.outTradeNo)
            }
          }).catch(err=>{
            alert("请求失败")
          })
          ...
          this.axios.get(this.Global.hostUrl+'mobile/alipay/createorder',data,config).then((res) => {
            console.log(res.data);
            if(res.data.error=="success"){
              window.android.alipay(res.data.data.params)
            }
          }).catch(err=>{
            alert("请求失败")
          })
