		...
		<div class="pad-5 mar-t-1 bg-green">
			<div class="poster-list scroll-x">
				<div class="flex flex-start-x re">
					<img v-if="isShow" v-for="item,index in ranklist" @click="change(index)" :src="item.filename" alt="" width="100%"/>
				</div>
			</div>
		</div>
...
////             GET /someUrl
//          this.$http.get(this.Global.hostUrl+'mobile/haibao/index',data,config).then(res => {
...
//          })
			...
			this.ranklist=[{filename:poster1},{filename:poster2},{filename:poster3},{filename:poster4}]
