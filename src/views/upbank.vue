<template>
    <div>
    <headernav headTip="上传银行卡" backUrl="./realName"></headernav>
    ...
    export default {
        ...
        mounted:function(){
            ...
            this.axios.get(this.Global.upUrl+'mobile/yeepaySkbSdk/findUser',data,config).then(res=>{
                ...
                if(res.data.data.bankImg !=''){
                    this.img=this.Global.upUrl+res.data.data.bankImg;
                    this.state=1;
                }
        ...
        methods:{
            ...
            next(e){
                if(this.state==1){
                    this.$router.push({path:'/upidcard1'})
                }else{
                    alert('请上传银行卡图片');
                }
