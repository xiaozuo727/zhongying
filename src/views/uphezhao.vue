<template>
    <div>
    <headernav headTip="上传合照" backUrl="./upidcard2"></headernav>
    ...
    export default {
        ...
        mounted:function(){
            ...
            this.axios.get(this.Global.upUrl+'mobile/yeepaySkbSdk/findUser',data,config).then(res=>{
                ...
                if(res.data.data.AllImg !=''){
                    this.img=this.Global.upUrl+res.data.data.AllImg;
                    this.state=1;
                }
            }).catch(err=>{
                // alert('请求失败')
        ...
        methods:{
            ...
            next(e){
                ...
                if(this.state==1){
                    ...
                    data.params.randpwd=localStorage.token;
                    var config= {headers: {'X-Requested-With': 'XMLHttpRequest'},emulateJSON: true};
                    this.axios.get(this.Global.upUrl+'mobile/yeepaySkbSdk/reg',data,config).then(res=>{
                        console.log(res.data);
