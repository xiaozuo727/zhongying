<template>
    <div>
    <headernav headTip="上传身份证件" backUrl="./zn_upbank"></headernav>
    ...
    <div class="content">
        <div class="pad-l-r-1">
            ...
            <div class="center mar-t-1">
                <img src="../assets/img/example2.jpg" alt="" width="100%" v-model="imgData">
            </div>
        </div>
...
<script>
    ...
    export default {
        data(){
            return{
                img:require('../assets/img/upfile2.png'),
                imgData:{
        ...
        mounted:function(){
            ...
            this.axios.get(this.Global.upUrl+'mobile/znhk/findUser',data,config).then(res=>{
                ...
                if(res.data.data.user_IDcardFront !=''){
                    this.img=this.Global.upUrl+res.data.data.user_IDcardFront;
                    this.state=1;
                }
            }).catch(err=>{
                // alert('请求失败')
        ...
        methods:{
            ...
            next(e){
                if(this.state==1){
                    this.$router.push({path:'/zn_upidcard2'})
                }else{
                    alert('请上传身份证正面照');
                }
