<template>
    <div>
    <headernav headTip="上传身份证件" backUrl="./zn_upidcard1"></headernav>
    ...
    export default {
        ...
        mounted:function(){
            ...
            this.axios.get(this.Global.upUrl+'mobile/znhk/findUser',data,config).then(res=>{
                ...
                if(res.data.data.user_IDcardBack !=''){
                    this.img=this.Global.upUrl+res.data.data.user_IDcardBack;
                    this.state=1;
                }
            }).catch(err=>{
                // alert('请求失败')
        ...
        methods:{
            ...
            next(e){
                if(this.state==1){
                    this.$router.push({path:'/zn_uphezhao'})
                }else{
                    alert('请上传身份证背面照');
                }
