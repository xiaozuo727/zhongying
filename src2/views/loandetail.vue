<template>
	<div>
	<headernav headTip="贷款详情" backUrl="./loan"></headernav>
	...
	<div class="content">
				...
				<div class="flex">
					...
					<div class="pad-l-1">
						...
						<div class="color-gray f12">
							...
							<div>可贷款额度： <span class="color-red">{{item.scoped}}</span></div>
						</div>
					</div>
				</div>
		...
		<div class="flex f16 pad-t-b-1 line-height-2">
			...
			<div class="btn bg-green col-4" @click="apply">立即申请</div>
		</div>
...
//			console.log(parseFloat(this.$route.query.item.reward));
			...
			var sign=pat.test(this.$route.query.item.reward)?"%":"元";
			var jin=parseFloat(this.$route.query.item.reward);
			this.lv1=(jin*0.6).toFixed(2)+sign;
			this.lv2=(jin*0.85).toFixed(2)+sign;
			this.lv3=(jin*0.95).toFixed(2)+sign;
